...
% XXX Justificar por que tem essas instruções JIT_*
...
Figura \ref{fig:limiar} é tomado como base para a discussão.
 %Para um caso
%específico será possível verificar um erro em potencial durante a execução
%de código.

%O exemplo da Figura \ref{fig:global1} é bastante simples,
%porém incorreto na linguagem \texttt{Tcl}, pois é necessário preceder
%explicitamente a variável de seu escopo. Na \texttt{Tcl} o escopo global é
%simbolizado por ``\verb!::!'', além disto também é possível utilizar o comando
...
% \subsubsection{Código de máquina IA-32} % Anderson marcou como talvez
%                                 % não necessário
...
%instrução \verb!PUSH! é um registrador, indicado por ``reg''.
 Para a
...
multiplicação.
% simples como a \verb!JIT_INST_INCR! requer uma
%quantidade relativamente alta de bytes para codificá-la e, portanto,
%é possível utilizá-la para uma discussão detalhada.
...
%  O comando \verb!incr! da \texttt{Tcl} permite
...
% for 1. Para os demais casos uma instrução \verb!JIT_INST_ADD! é utilizada.
% Este exemplo utiliza o comportamento padrão do comando \verb!incr!, que
% incrementa em uma unidade a variável indicada.
...
% A instrução \verb!JIT_INST_INCR! sendo corretamente
% codificada, o valor do operando A (campo \verb!src_a! da estrutura
% \verb!Quadruple! da Figura \ref{quad-struct}) contém uma
% indicação de que o operando é uma variável local. % Sendo assim,
% %precisamos chegar a essa variável local.
% A Seção \ref{codeexec} % foi
% descreveu a assinatura da função criada pelo compilador JIT,
% sem detalhas os tipos dos argumentos.
% % mas ainda
...
% execução, o primeiro parâmetro é uma estrutura
% interna \verb!Interp! que será utilizada para acessar a
% variável local. A partir desta, os primeiros bytes
% gerados são referentes a movimentação do primeiro parâmetro para
% um registrador.
\begin{figure}[ht!]
  \centering
  \begin{lstlisting}[language=C]
...
% XXX COlocar isso em algum lugar
% Como é feito a suposição de
%que os valores serão inteiros, também é feito uma tentativa de
%conversão de \verb!Tcl_Obj! para inteiro aqui.
...
%que não há mais quádruplas ou mesmo outros blocos a serem visitados
a função
...
reutilizar código para outras instruções. % Uma
